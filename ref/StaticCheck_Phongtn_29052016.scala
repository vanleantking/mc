...
class StaticChecker(ast:AST) {
    def  check() = {
      try {
        val ce = new GlobalGeneration();     
        val env = ce.visit(ast, null).asInstanceOf[SymbolList]
        val cen = new ClassEnvironment();
        val clenv = cen.visit(ast, null).asInstanceOf[GlobalSymbolList]
        val tc = new TypeChecking(clenv)
        tc.visit(ast, null)
        
//        val global = ce.visit(ast,null).asInstanceOf[List[ClDecl]]
//        for(astchild <- global) {
//          if (global.exists(x => x.name == astchild.name.toString())) throw Redeclared(Class, astchild.name.toString())
//        }
      } catch {
...
//case class ClDecl(name:String,parent:String,mem:List[MDecl])
//
//
//
//trait MDecl
//case class MuField(variable:String,varType:Type) extends MDecl
//case class MethodT(name:String,input:List[Type], returnType:Type) extends MDecl
...
class ClassEnvironment extends BaseVisitor {
  ...
  override def visitVarDecl(ast: VarDecl, c: Context) = {
      val env = c.asInstanceOf[SymbolList].list
      val newenv = if (env.exists(x => x._1 == ast.variable.toString())) throw Redeclared(Variable, ast.variable.toString()) else (ast.variable.toString(), ast.varType, Variable, Instance) :: env
      SymbolList(newenv)
...
class GlobalGeneration extends BaseVisitor {
  
  
//  override def visitProgram(ast: Program, c: Context): Object = ast.decl.map(visit(_, null)).asInstanceOf[List[ClDecl]]
//  override def visitVarDecl(ast: VarDecl, c: Context): Object = MuField("",IntType)
//  override def visitConstDecl(ast: ConstDecl, c: Context): Object =null
//  override def visitParamDecl(ast: ParamDecl, c: Context): Object = ast.paramType
//  override def visitClassDecl(ast: ClassDecl, c: Context): Object = {
//		val mem = ast.decl.map(visit(_, null)).asInstanceOf[List[MDecl]]		
//		val varmem = mem.filter(_.isInstanceOf[MuField])
//		val methodmem = mem.filter(_.isInstanceOf[MethodT])
//		
//		ClDecl(ast.name.name,(if (ast.parent != null) ast.parent.name else ""),ast.decl.map(visit(_, null)).asInstanceOf[List[MDecl]])
		
		//		val globalList = c.asInstanceOf[SymbolList].list
//		if (globalList.exists(x => x.name == ast.name.toString())) throw Redeclared(Class, ast.name.toString())
		//checkDuplicate(mem,)
		//if (mem.exists(x => x.name == ast.name.toString())) throw Redeclared(Class, ast.name.toString())
		
//		for(m1 <- varmem) {
//            if (varmem.exists(x => x.name == m1.name)) throw Redeclared(Variable, m1.asInstanceOf[MuField].name.toString())
//    }
//
//    for(m2 <- methodmem) {
//            if (varmem.exists(x => x.name == m2.name)) throw Redeclared(Variable, m2.asInstanceOf[MethodT].name.toString())
//        
//		}
		
//	}
//  override def visitMethodDecl(ast: MethodDecl, c: Context): Object = MethodT(ast.name.name,ast.param.map(visit(_, null)).asInstanceOf[List[Type]],ast.returnType)
...
class TypeChecking(clenv: GlobalSymbolList) extends BaseVisitor with Utils {
  ...
  override def visitId(ast: Id, c: Context) = {
    ...
    //val newenv = if (env.exists(x => x._1 == id.toString())) throw Redeclared(Variable, id.toString()) 
  ...
  override def visitFor(ast: For, c: Context) = {
    val expr = visit(ast.expr1, c).asInstanceOf[(Type, Kind)]
